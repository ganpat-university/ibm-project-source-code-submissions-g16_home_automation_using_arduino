...
if (inputs.length() >0)
{
  ...
  if(inputs == "A")
  {
    digitalWrite(relay1, LOW);     //when we press on on bluetooth device A is passed and relay is set to low that is high
  }
  else if(inputs == "a")
  {
    digitalWrite(relay1, HIGH);             //when off is pressed
  }
  else if(inputs == "B")
  {
